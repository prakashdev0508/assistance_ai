...
const STATE_COOKIE_NAME = "gg_oauth_state";
...
export async function GET(request: NextRequest) {
  ...
  if (!session?.user?.email) {
    return NextResponse.redirect(new URL("/login?intent=gmail", baseRedirect));
  }
  ...
  const existing = await getGoogleGmailIntegration(user.id);
  ...
  await db.integration.upsert({
    ...
    update: {
      ...
      refreshToken,
      scope: tokenJson.scope ?? GOOGLE_GMAIL_SCOPES,
      expiresAt,
    },
    create: {
      userId: user.id,
...
function redirectWithStatus(baseUrl: URL, status: string) {
  baseUrl.searchParams.set("gmail", status);
...
function buildCallbackUrl(request: NextRequest) {
  ...
  return `${baseUrl}/api/integrations/google/gmail/callback`;
}
