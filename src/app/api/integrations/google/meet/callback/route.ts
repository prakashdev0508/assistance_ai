...
export async function GET(request: NextRequest) {
  ...
  if (!session?.user?.email) {
    return NextResponse.redirect(new URL("/login?intent=meet", baseRedirect));
  }
  ...
  const existing = await getGoogleMeetIntegration(user.id);
  ...
  await db.integration.upsert({
    ...
    update: {
      ...
      refreshToken,
      scope: tokenJson.scope ?? GOOGLE_MEET_SCOPES,
      expiresAt,
    },
    create: {
      userId: user.id,
...
function redirectWithStatus(baseUrl: URL, status: string) {
  baseUrl.searchParams.set("meet", status);
